...
| handheld | subject containment | 100.0% | >= 99% of samples | PASS | |center err| <= crop_w/2-40px; crop_w=606 |
| handheld | mean tracking error | 26px | <= crop_w/4 = 152px | PASS |  |
| handheld | tracking loss rate | 0.0px avg | <= 2px avg overflow | PASS |  |
| music | snr18: stage gates | pass | no StageError | PASS |  |
| music | snr18: silence false-positive rate | 0.00% of speech time | <= 1% | PASS |  |
| music | snr18: pause recall | 100% (4/4) | >= 80% | PASS | threshold used: -16.0dB |
| music | snr08: stage gates | pass | no StageError | PASS |  |
| music | snr08: silence false-positive rate | 0.00% of speech time | <= 1% | PASS |  |
| music | snr08: pause recall | 100% (4/4) | >= 80% | PASS | threshold used: -10.6dB |
| alternation | stage gates (ingest+edit) | pass | no StageError | PASS |  |
| alternation | worst re-acquire after switch | 1.07s | <= 1.5s | PASS |  |
...
| low_contrast | detection share (face+motion) | 99.1% | >= 50% (motion fallback must carry) | PASS | face 35.7% / motion 0.9% / hold 0.9% |
| low_contrast | mean tracking error | 22px | <= 150px | PASS |  |
...
| boundary_silence | edited duration vs config-derived expectation | 29.18s | 29.47s ± 0.7s | PASS | keep_pad=0.15, min_cut=0.12 read from Config |
| boundary_silence | leading silence trimmed to t=0 edge rule | first kept at 2.85s | 2.85s ± 0.3s | PASS |  |
| boundary_silence | trailing silence trimmed to clip end | last kept ends 40.53s | 40.62s ± 0.3s | PASS |  |
| boundary_silence | pacing guard: min kept segment | 7.25s | >= min_segment 0.6s | PASS |  |
